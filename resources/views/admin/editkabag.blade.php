        ...
        <div id="content">
            ...
            <div class="container-fluid">
                ...
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">Edit :{{ $kabag->name }}</h1>
                </div>

                <form action="{{ route('editkabag', ['id'=>$kabag->id]) }}" class="m-5" method="POST">
                    @csrf
                    <div class="form-group">
                        <label for="id">ID :</label>
                        <input type="text" name="id" class="form-control form-control-user"
                            id="id"
                            placeholder="Enter Name..." value="{{ $kabag->id }} " disabled>
                    </div>
                    <div class="form-group">
                        <label for="name">Nama :</label>
                        <input type="text" name="name" class="form-control form-control-user"
                            id="name"
                            placeholder="Enter Name..." value="{{ $kabag->name }}">
                    </div>
                    <div class="form-group">
                        <label for="username">Username / NIDN :</label>
                        <input type="text" name="username" class="form-control form-control-user"
                            id="username" disabled
                            placeholder="Enter NIDN as Username..." value="{{ $kabag->username }}">
                    </div>
                    <div class="form-group">
                        <label for="password">Password :</label>
                        <input type="password" name="password" class="form-control form-control-user"
                            id="password" placeholder="Password lama" disabled value="{{ $kabag->password }}">
                    </div>
                    <div class="form-group">
                        <input type="password" name="password" class="form-control form-control-user"
                ...
                <a href="{{ route('ListKabag') }}" class="btn btn-danger col-xl mt-2">back</a>
            </div>
        </div>
    </div>
